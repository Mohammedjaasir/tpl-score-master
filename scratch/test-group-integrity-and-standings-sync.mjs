...
// TEST 1: Initial state - 0 matches played, 0 points for all teams
const initialStandings = calculateStandings(lookup.teams(), lookup.matches());
assert.strictEqual(initialStandings.length, 6);
initialStandings.forEach((st) => {
  assert.strictEqual(st.played, 0, `${st.teamName} played must be 0`);
  assert.strictEqual(st.won, 0, `${st.teamName} won must be 0`);
  assert.strictEqual(st.lost, 0, `${st.teamName} lost must be 0`);
  assert.strictEqual(st.points, 0, `${st.teamName} points must be 0`);
  assert.strictEqual(st.nrr, 0, `${st.teamName} NRR must be 0`);
});
console.log("  ✓ TEST 1: Initial Standings are completely zeroed (P:0, W:0, L:0, PTS:0, NRR:0.00).");
...
console.log("  ✓ TEST 4: Schedule generation strictly preserves master Group 1 / Group 2 assignments.");
...
assert.strictEqual(unplayedStats?.played, 0, "DU played 0 matches");
assert.strictEqual(unplayedStats?.points, 0, "DU has 0 points");
...
// TEST 9: Reset All Matches returns Standings to zero and preserves master teams/groups
lookup.setMatches([]);
const resetStandings = calculateStandings(lookup.teams(), lookup.matches());

resetStandings.forEach((st) => {
...
console.log("  ✓ TEST 9: Reset All Matches resets all standings to 0 while strictly preserving teams, groups, and players.");
