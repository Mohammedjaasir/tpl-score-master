...
// ── Team Crest (compact, used in mobile row) ─────────────────────────────────

function TeamCrest({ teamId }: { teamId: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 text-center min-w-0 w-[72px] sm:w-[88px] md:w-[110px]">
      <div className="h-11 w-11 sm:h-14 sm:w-14 md:h-16 md:w-16 shrink-0 rounded-xl bg-black/50 border border-white/15 p-1 flex items-center justify-center shadow-md group-hover:border-[#D9A928]/40 transition-colors">
        {team?.logoUrl ? (
      ...
      <div className="leading-tight">
        <p className="text-[9px] sm:text-[10px] font-semibold text-white/65 uppercase tracking-wider truncate w-[72px] sm:w-[88px] md:w-[110px]">
          {line1}
        </p>
        <p className="text-[10px] sm:text-xs font-black text-white uppercase tracking-wide truncate w-[72px] sm:w-[88px] md:w-[110px]">
          {line2 || line1}
        </p>
      </div>
    </div>
    ...
    <article className="group relative overflow-hidden rounded-2xl bg-[#121316] border border-white/[0.12] shadow-2xl transition-all duration-300 hover:border-[#D9A928]/40">
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-center gap-3 px-4 pb-3 md:pb-4">
          ...
          {/* Center: Head-to-head teams */}
          <div className="flex-1 flex items-center justify-center gap-2 sm:gap-3">
            <TeamCrest teamId={match.teamAId} />
            ...
            <TeamCrest teamId={match.teamBId} />
          </div>
        ...
        {isDone && match.resultText && (
          <div className="border-t border-white/10 bg-black/40 px-4 py-2 text-center">
            <p className="text-[10px] font-black text-[#D9A928] uppercase tracking-wide">{match.resultText}</p>
          </div>
        )}
      </div>
    </article>
