...
function LandingScreen() {
  ...
  return (
    <div className="min-h-screen w-full bg-white text-[#0A0A0A] font-sans selection:bg-[#D9A928] selection:text-black overflow-x-hidden">
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          3. LIVE MATCH (Rendered ABOVE NEXT MATCH)
          ══════════════════════════════════════════════════════════════════════ */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            ...
            <div className="rounded-3xl border border-[#D9A928]/30 bg-[#111] p-6 sm:p-8">
              ...
              <div className="grid grid-cols-3 gap-4 items-center">
                <div>
                  ...
                  <p className="text-3xl sm:text-4xl font-black text-white mt-2">
                    {liveState?.currentInnings?.runs ?? (liveMatch ? 0 : 142)}
                    <span className="text-white/40 text-xl">
                      /{liveState?.currentInnings?.wickets ?? (liveMatch ? 0 : 4)}
                    </span>
                  </p>
                  <p className="text-[11px] text-white/40 mt-1">
                    {liveState?.currentInnings?.overs ?? (liveMatch ? "0.0" : "17.2")} Overs
                  </p>
                </div>
              ...
              <div className="mt-5 pt-4 border-t border-white/10 flex items-center justify-between text-[11px] text-white/40">
                <span>CRR {liveState?.currentInnings?.crr ?? (liveMatch ? "0.00" : "8.19")}</span>
                <span className="text-[#D9A928] font-bold">
                  Partnership {liveState?.currentInnings?.partnershipRuns ?? (liveMatch ? 0 : 48)} ({liveState?.currentInnings?.partnershipBalls ?? (liveMatch ? 0 : 32)})
                </span>
              </div>
