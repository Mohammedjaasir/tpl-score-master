...
import { useMatches, usePrefetchCricketData } from "@/hooks/useCricketData";
...
function LandingScreen() {
  ...
  // 1. Authoritative LIVE match check (status === "LIVE")
  const liveMatch = allMatches.find(
    (m) => (m.status || "").toUpperCase() === "LIVE"
  );

  // 2. Authoritative NEXT match check (status === "UPCOMING" || "READY" || "SCHEDULED")
  const upcomingMatches = allMatches
    .filter((m) => {
      const s = (m.status || "").toUpperCase();
      return s === "UPCOMING" || s === "READY" || s === "SCHEDULED";
    })
    .sort((a, b) => {
      const timeA = new Date(a.scheduledAt || (a as any).startTime || 0).getTime();
      const timeB = new Date(b.scheduledAt || (b as any).startTime || 0).getTime();
    ...
    <div className="min-h-screen w-full bg-white text-[#0A0A0A] font-sans selection:bg-[#D9A928] selection:text-black overflow-x-hidden">
      ...
      {liveMatch && (() => {
        ...
        if (typeof window !== "undefined") {
          try {
            const raw = window.localStorage.getItem("tpl-scoring:" + liveMatch.id);
        ...
        const currentInnings = computed?.currentInnings;
        const inn1 = computed?.innings[0];
        const inn2 = computed?.innings[1];

        const battingTeamId = currentInnings?.battingTeamId || liveMatch.teamAId;
        const bowlingTeamId = currentInnings?.bowlingTeamId || liveMatch.teamBId;
        ...
        const crrStr = computed?.crr || "0.00";
        const partnershipStr = computed?.partnership
          ? `Partnership ${computed.partnership.runs} (${computed.partnership.balls})`
          : inn1 && inn2
          ? `Target ${inn2.target ?? (inn1.runs + 1)}`
          : "Match in Progress";

        const bowlingScoreStr = inn2 && inn1
                ...
                <div className="rounded-3xl border border-[#D9A928]/30 bg-[#111] p-6 sm:p-8">
                  ...
                  <div className="grid grid-cols-3 gap-4 items-center">
                    <div>
                      ...
                      <p className="text-xl sm:text-2xl font-black text-white uppercase leading-tight">{battingTeam?.name || liveMatch.teamAId}</p>
                      <p className="text-3xl sm:text-4xl font-black text-white mt-2">{runs}<span className="text-white/40 text-xl">/{wickets}</span></p>
                      <p className="text-[11px] text-white/40 mt-1">{oversStr} Overs</p>
                    </div>
                    <div className="text-center">
                      <span className="inline-block px-3 py-2 rounded-xl bg-white/10 text-white text-sm font-black">VS</span>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-1">{inn2 ? "1ST INNINGS" : "YET TO BAT"}</p>
                      <p className="text-xl sm:text-2xl font-black text-white uppercase leading-tight">{bowlingTeam?.name || liveMatch.teamBId}</p>
                      <p className="text-xl font-black text-white/40 mt-2">{bowlingScoreStr}</p>
                    </div>
                  </div>

                  <div className="mt-5 pt-4 border-t border-white/10 flex items-center justify-between text-[11px] text-white/40">
                    <span>CRR {crrStr}</span>
                    <span className="text-[#D9A928] font-bold">{partnershipStr}</span>
                  </div>
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          ...
          ══════════════════════════════════════════════════════════════════════ */}
      {nextMatch ? (() => {
        const teamA = lookup.team(nextMatch.teamAId);
        const teamB = lookup.team(nextMatch.teamBId);
        return (
            ...
            <div className="max-w-7xl mx-auto px-5 lg:px-8">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-6 sm:gap-10">
                  <div className="text-center">
                    <p className="text-2xl sm:text-3xl font-black text-black uppercase">{teamA?.name || nextMatch.teamAId}</p>
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[9px] text-black/50 font-black mt-2 uppercase tracking-wider">
                      {nextMatch.scheduledAt ? formatMatchTime(nextMatch.scheduledAt) : "Today"}
                    </p>
                    <p className="text-[9px] text-black/40 font-bold uppercase">TPL 2026 · Match {nextMatch.matchNumber || 1}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl sm:text-3xl font-black text-black uppercase">{teamB?.name || nextMatch.teamBId}</p>
        ...
        <section className="py-14 bg-[#D9A928]">
          <div className="max-w-7xl mx-auto px-5 lg:px-8">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
              <div>
                <p className="text-[10px] font-black tracking-[0.28em] text-black/60 uppercase mb-2">TOURNAMENT MATCHES</p>
                <h2 className="font-display font-black text-3xl sm:text-4xl uppercase leading-tight text-black">
                  FOLLOW TPL FOR UPCOMING FIXTURES
                </h2>
              </div>
              <Link
                to="/matches"
