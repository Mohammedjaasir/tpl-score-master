...
function LiveMatchesPage() {
  ...
  const sections = [
    { label: "Live Matches in Progress", matches: live, isLive: true },
    { label: "Ready to Start", matches: ready, isLive: false },
    { label: "Upcoming Fixtures", matches: upcoming, isLive: false },
    { label: "Recent Results", matches: completed, isLive: false },
  ].filter((s) => s.matches.length > 0);

  return (
    <AppShell title="Live Scores">
      <div className="flex flex-col gap-6 max-w-4xl mx-auto pt-2 pb-16">
        <div className="flex items-center justify-between px-1">
          ...
          {live.length > 0 && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-500/10 text-red-600 border border-red-500/20 text-xs font-black uppercase tracking-wider">
        ...
        {sections.map((s) => (
          <section key={s.label} className="flex flex-col gap-3">
            <h2 className="text-xs font-black tracking-widest text-[#5F6368] uppercase px-1 flex items-center gap-2">
              {s.isLive && <span className="h-1.5 w-1.5 rounded-full bg-red-500 animate-pulse" />}
              {s.label}
            </h2>
